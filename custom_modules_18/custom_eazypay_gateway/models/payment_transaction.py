...
class PaymentTransaction(models.Model):
    ...
    def _eazypay_get_bhd_amount_currency(self):
        """Return (amount, currency) in BHD for EazyPay API.

        EazyPay only supports BHD. When the transaction (or linked sale order) is in
        another currency (e.g. SAR from a pricelist), use the company/default currency
        (BHD) and convert the amount so the gateway accepts the request.

        :return: (amount: float, currency: res.currency)
        :raises: ValidationError if BHD currency is not available
        """
        self.ensure_one()
        company = self.company_id or self.provider_id.company_id
        bhd = self.env.ref('base.BHD', raise_if_not_found=False) or self.env['res.currency'].search(
            [('name', '=', 'BHD')], limit=1
        )
        if not bhd:
            # Fallback: company currency if it is typically the "default" (e.g. Bahrain)
            company_curr = company.currency_id
            if company_curr.name == 'BHD':
                bhd = company_curr
            else:
                raise ValidationError(_(
                    "EazyPay only supports BHD. Please configure Bahraini Dinar (BHD) as a currency."
                ))
        if self.currency_id == bhd:
            return (self.amount, bhd)
        # Convert to BHD: use amount from sale orders converted to BHD (default pricelist = company BHD)
        if getattr(self, 'sale_order_ids', None) and self.sale_order_ids:
            total_bhd = 0.0
            for order in self.sale_order_ids:
                order_total = order.currency_id._convert(
                    order.amount_total,
                    bhd,
                    company,
                    fields.Date.today(),
                ...
                total_bhd += order_total
            return (total_bhd, bhd)
        amount_bhd = self.currency_id._convert(
            self.amount,
            bhd,
            company,
            fields.Date.today(),
    ...
    def _eazypay_create_invoice(self):
        ...
        # EazyPay only supports BHD: use amount/currency from default (BHD) pricelist
        amount_bhd, currency_bhd = self._eazypay_get_bhd_amount_currency()
        amount_str = f"{amount_bhd:.3f}"
