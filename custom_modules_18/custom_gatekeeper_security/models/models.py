...
class ServiceRequest(models.Model):
    ...
    def create(self, vals):
        """Ensure sequence is generated for new service requests"""
    ...
    def _compute_customer_info(self):
        for record in self:
            ...
            if partner:
                # Only compute company_partner_id if not already set (allows manual override)
                if not record.company_partner_id:
                    record.company_partner_id = partner.parent_id or False
                # Only compute service_address if it's empty or matches the partner's address
                # This allows manual override during create
                if not record.service_address:
                    record.service_address = partner._display_address()
                record.city = partner.city or ''
                # Only compute state_id if not already set (allows manual override)
                ...
                if not record.company_partner_id:
                    record.company_partner_id = False
                if not record.service_address:
