...
patch(PosStore.prototype, {
    async addLineToOrder(vals, order, opts = {}, configure = true) {
        ...
        if (product.isTracked() && options.lots) {
            lotMatch = options.lots.find((lot) => lot.lot_name && lot.cost_price);
            if (lotMatch) {
                values.price_unit = lotMatch.cost_price; // Initial assignment
            }
        }
        ...
        if (product.isTracked() && lotMatch) {
            line.price_unit = lotMatch.cost_price;
            line.tax_ids = [];
        }
        if (configure) {
        ...
        if (product.isTracked()) {
            ...
            // ✅ If tracked product, fetch the lot and use cost_price as price_unit
            if (pack_lot_ids.newPackLotLines?.length) {
                const selectedLotName = pack_lot_ids.newPackLotLines[0].lot_name;

                // Call backend again to fetch lot info including cost_price
                const lotInfo = await this.data.call(
                    "pos.order.line",
                    "get_existing_lots",
                ...
                const lotMatch = lotInfo.find((lot) => lot.name === selectedLotName);
                if (lotMatch) {
                    line.price_unit = lotMatch.cost_price;
